      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#141414] border border-white/10 rounded-2xl p-8 shadow-2xl">
          {step === "email" && (
            <>
              ...
              <div className="relative mb-6">
                ...
                <div className="relative flex justify-center text-sm"><span className="px-2 bg-[#141414] text-white/40">Or sign in with email</span></div>
              </div>
            ...
            <>
              ...
              <form onSubmit={handleVerifyOTP} className="space-y-6">
                ...
                <Button type="submit" disabled={isLoading || otp.some((d) => !d)} className="w-full h-12 bg-[#D9FC67] hover:bg-[#E8FF8A] text-black font-semibold rounded-full disabled:opacity-50">
                  {isLoading ? "Verifying…" : "Verify & Sign In"} <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
              </form>
          ...
          <p className="mt-6 text-center text-sm text-white/60">
            ...
            <Link href="/partner/signup" className="text-[#D9FC67] hover:text-[#E8FF8A] font-medium transition-colors">Apply now</Link>
          </p>
        </div>

        <div className="text-center mt-6">
          <Link href="/auth/login" className="text-sm text-white/40 hover:text-white/60 transition-colors">Customer Login →</Link>
        </div>
      </div>
    </div>
  );

...
import { Mail, User, Phone, ArrowRight, Check, RotateCcw, Building2 } from "lucide-react";
...
function PartnerSignupContent() {
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    const result = await signIn("credentials", { email, token: verificationToken, rememberMe: String(rememberMe), redirect: false });
    if (result?.error) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#141414] border border-white/10 rounded-2xl p-8 shadow-2xl">
          ...
          {step === "profile" && (
            <>
              ...
              <form onSubmit={handleProfileSubmit} className="space-y-4">
                ...
                <Button type="submit" disabled={isLoading || !name || !businessName} className="w-full h-12 bg-[#D9FC67] hover:bg-[#E8FF8A] text-black font-semibold rounded-full disabled:opacity-50">
          ...
          <p className="mt-6 text-center text-sm text-white/60">
            ...
            <Link href="/partner/login" className="text-[#D9FC67] hover:text-[#E8FF8A] font-medium transition-colors">Sign in</Link>
          </p>
        </div>

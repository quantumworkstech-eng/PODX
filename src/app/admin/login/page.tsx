...
export default function AdminLoginPage() {
  ...
  const handleOtpKeyDown = (index: number, e: React.KeyboardEvent) => {
    if (e.key === "Backspace" && !otp[index] && index > 0) document.getElementById(`admin-otp-${index - 1}`)?.focus();
  };
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    const result = await signIn("credentials", { email, token: verifyData.token, redirect: false });
    if (result?.error) {
    ...
    <div className="min-h-screen bg-[#09090b] flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#141414] border border-white/10 rounded-2xl p-8">
          ...
          <p className="text-white/40 text-sm mb-6">Access restricted to authorized administrators only</p>

          {/* Google Sign In */}
          <button type="button" onClick={handleGoogleSignIn} disabled={loading}
            className="w-full h-12 flex items-center justify-center gap-3 bg-white/5 border border-white/10 rounded-xl text-white hover:bg-white/10 hover:border-white/20 transition-colors mb-4 disabled:opacity-50">
          ...
          <div className="relative mb-4">
            ...
            <div className="relative flex justify-center text-xs"><span className="px-2 bg-[#141414] text-white/30">or sign in with email OTP</span></div>
          </div>
            ...
            <form onSubmit={handleSendOTP} className="space-y-4">
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : (
                  <><Shield className="w-4 h-4" /> Send verification code <ArrowRight className="w-4 h-4" /></>
                )}
              </button>
            </form>
          )}
          ...
          {step === "otp" && (
            <>
              <p className="text-white/60 text-sm mb-4 text-center">
                We sent a 6-digit code to <span className="text-red-400 font-medium">{email}</span>
              </p>
              <form onSubmit={handleVerifyOTP} className="space-y-4">
                <div className="flex justify-center gap-2">
                  {otp.map((digit, index) => (
                    <input key={index} id={`admin-otp-${index}`} type="text" inputMode="numeric" value={digit} maxLength={1}
                      onChange={(e) => handleOtpChange(index, e.target.value)} onKeyDown={(e) => handleOtpKeyDown(index, e)}
                      className="w-12 h-14 text-center text-2xl font-bold bg-white/5 border border-white/10 rounded-xl text-white focus:border-red-500/50 focus:outline-none transition-colors" />
                  ))}
                </div>
              ...
              <button type="button" onClick={() => { setStep("email"); setOtp(["","","","","",""]); setError(""); }}
                className="w-full mt-3 flex items-center justify-center gap-2 text-sm text-white/30 hover:text-white/50 transition-colors">
                <RotateCcw className="w-4 h-4" /> Use different email or resend
              </button>
            </>
          )}
        </div>
